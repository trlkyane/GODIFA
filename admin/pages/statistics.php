...
if (isset($_GET['error'])) {
    switch ($_GET['error']) {
        case 'permission_denied':
            $errorMessage = '🚫 Bạn không có quyền truy cập trang Chỉ có chủ doanh nghiệp mới có thể truy cập.';
            break;
        case 'unauthorized':
...
if ($errorMessage): ?>
            ...
            <div class="flex flex-row justify-between items-start gap-4">
                <div>
                    ...
                    <p class="text-sm text-gray-600 mt-1">Tiêu chí: <?php
echo $periodLabel; ?></p>
                </div>
...
echo $period == 'day' ? 'selected' : ''; ?>>Hôm nay</option>
                        <option value="7days" <?php
echo $period == '7days' ? 'selected' : ''; ?>>7 ngày trước</option>
                        <option value="30days" <?php
echo $period == '30days' ? 'selected' : ''; ?>>30 ngày trước</option>
                        <option value="month" <?php
echo $period == 'month' ? 'selected' : ''; ?>>Tháng này</option>
                        <option value="lastmonth" <?php
echo $period == 'lastmonth' ? 'selected' : ''; ?>>Tháng trước</option>
                        <option value="custom" <?php
echo $period == 'custom' ? 'selected' : ''; ?>>Tùy chỉnh</option>
                    </select>
                    <input type="date" id="startDate" value="<?php
echo $startDate; ?>" 
                           class="px-3 py-2 border rounded-lg">
                    <span>đến</span>
                    <input type="date" id="endDate" value="<?php
echo $endDate; ?>" 
                           class="px-3 py-2 border rounded-lg">
            ...
            <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg shadow-lg p-4 text-white">
                <div class="flex justify-between items-start mb-3">
                    <div>
                        ...
                        <h3 class="text-xl font-bold mt-1"><?php
echo $controller->formatCurrency($revenueStats['total']); ?></h3>
                    </div>
            ...
            <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow-lg p-4 text-white">
                <div class="flex justify-between items-start mb-3">
                    <div>
                        ...
                        <h3 class="text-xl font-bold mt-1"><?php
echo $controller->formatCurrency($revenueStats['completed']); ?></h3>
                    </div>
                    <div class="bg-white bg-opacity-20 rounded-full p-2">
                ...
                <p class="text-green-100 text-xs">
                    <?php
echo number_format(($revenueStats['total'] > 0 ? ($revenueStats['completed']/$revenueStats['total'])*100 : 0), 1); ?>% tổng doanh thu
                </p>
            </div>
            ...
            <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-lg shadow-lg p-4 text-white">
                <div class="flex justify-between items-start mb-3">
                    <div>
                        ...
                        <h3 class="text-xl font-bold mt-1"><?php
echo $controller->formatCurrency($revenueStats['pending']); ?></h3>
                    </div>
                    <div class="bg-white bg-opacity-20 rounded-full p-2">
                ...
                <p class="text-yellow-100 text-xs">
                    <?php
echo number_format(($revenueStats['total'] > 0 ? ($revenueStats['pending']/$revenueStats['total'])*100 : 0), 1); ?>% tổng doanh thu
                </p>
            </div>
            ...
            <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg shadow-lg p-4 text-white">
                <div class="flex justify-between items-start mb-3">
                    <div>
                        ...
                        <h3 class="text-xl font-bold mt-1"><?php
echo number_format($orderStats['total']); ?></h3>
                    </div>
                    <div class="bg-white bg-opacity-20 rounded-full p-2">
...
echo date('d/m/Y', strtotime($startDate)); ?> - <?php
echo date('d/m/Y', strtotime($endDate)); ?>)</span>
                <?php
else: ?>
                    <span class="text-xs text-gray-500">(<?php
echo $periodLabel; ?>)</span>
                <?php
endif; ?>
            </h2>
...
echo number_format($product['totalSold']); ?> sản phẩm</p>
                            ...
                            <div class="text-right">
                                <p class="font-bold text-sm text-green-600"><?php
echo $controller->formatCurrency($product['revenue']); ?></p>
                            </div>
                        </div>
                        <?php
endforeach; ?>
                        ...
                        <div>
                            <div class="flex justify-between mb-1">
                                <span class="font-medium text-sm text-gray-700">
                                    <?php
echo htmlspecialchars($method['paymentMethod'] ?: 'Không xác định'); ?>
                                </span>
                                <span class="text-xs text-gray-600">
                                    <?php
echo number_format($method['orderCount']); ?> đơn
                                </span>
                            </div>
                            <div class="flex items-center gap-2">
                                <div class="flex-1 bg-gray-200 rounded-full h-2 overflow-hidden">
                                    <div class="<?php
echo $color; ?> h-full rounded-full transition-all" 
                                         style="width: <?php
echo $percentage; ?>%"></div>
                                </div>
                                <span class="text-xs font-semibold text-gray-900 w-20 text-right">
                                    <?php
echo $controller->formatCurrency($method['revenue']); ?>
                                </span>
                            </div>
...
<!-- JavaScript -->
<script>
    // --- 1. Chart.js Config (Gi? nguyên ph?n này) ---
    const chartData = <?php
echo json_encode($chartData); ?>;
    ...
    // --- 2. X? lý Logic b? l?c ngày (ÐÃ T?I UU) ---

    const startDateInput = document.getElementById('startDate');
    const endDateInput = document.getElementById('endDate');
    const periodSelect = document.getElementById('periodFilter');

    // Nếu người dùng sửa tay vào ô ngày -> T? d?ng chuy?n Dropdown sang "Tùy chỉnh"
    function switchToCustom() {
        periodSelect.value = 'custom';
    }
    
    // S? ki?n 'change' ch? kích ho?t khi người dùng thao tác, không kích ho?t khi JS d?i giá tr?
    startDateInput.addEventListener('change', switchToCustom);
    endDateInput.addEventListener('change', switchToCustom);

    // Hàm x? lý khi chọn Dropdown (Preset)
    function updateDateRange(period) {
        const today = new Date();
        let startDate, endDate;
        
        // Nếu ch?n custom thì d?ng, d? người dùng tự nhập
        ...
        // [QUAN TR?NG] Submit ngay lập tức v?i đúng PERIOD dó
        submitFilter(period, startDate, endDate);
    }

    // Hàm x? lý khi bấm nút "Áp d?ng"
    function applyCustomDate() {
        // Khi bấm nút, ta luôn coi là Custom (ho?c gi? giá tr? hi?n t?i c?a select)
        // Nhung an toàn nh?t là ép v? custom d? backend uu tiên ngày tháng
        // Tuy nhiên, Nếu dropdown dang là '7days' mà b?m Áp d?ng thì v?n nên gi? '7days'
    ...
    function exportToExcel() {
        ...
        // Tạo URL xuất file với BASE_URL
